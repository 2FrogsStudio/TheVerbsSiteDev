...
const onInstall = async () => {
    ...
    let urlsToCache = assertUrls.concat(cardImageUrl).concat(forceCacheUrls).filter(
...
const onActivate = async () => {
    ...
    await Promise.all(cacheKeys.filter(
        key => key.startsWith(cacheNamePrefix) && key !== cacheName).map(key => caches.delete(key)));
};
...
const getCardImageUrls = async () => {
    ...
    return Object.keys(cardsJson).map(key => cardsJson[key].image);
};

const getAssetUrls = async () => {
    // Fetch and cache all matching items from the assets manifest
    return self.assetsManifest.assets.filter(asset => offlineAssetsInclude.some(
        pattern => pattern.test(asset.url))).filter(asset => !offlineAssetsExclude.some(
        pattern => pattern.test(asset.url))).map(asset => asset.url);
};